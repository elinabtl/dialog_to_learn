...
# TODO: If handling an email fails - mark it as unread again.
...
def readEmailFromGmail():
  for classIndex, currentClass in enumerate(CLASS_DATA.classes):
    try:
      ...
      print("<<<Class: " + currentClass.class_email + " , unread emails: " +
            str(len(idList)) + ">>>\n")
      ...
      for idx in idList:
        ...
        for response_part in data:
          if isinstance(response_part, tuple):
            ...
            emailData = readEmail(msg, classIndex, currentClass.class_email)
            if emailData != None:
...
def readEmail(msg, classIndex, classEmail):
  ...
  if lastName == None and demoEmail == None:
    return None
  sendData = parseEmail(emailFrom, emailTo, emailSubject, emailContent,
                        lastName, classIndex, classEmail)
...
def composeEmail(emailData):
  ...
  # couldn't change the from address to contain +name, so added reply to option:
  msg['Subject'] = emailData.sendData.subject
...
def appendCensoredToText(input_text, censored_text):
  ...
  return " ".join([i[2:] if i[:1] == '-' or i[:1] == '+' else
                  i[2:] for i in dif])
...
def parseEmail(emailFrom, emailTo, subject, content, lastName, classIndex,
  ...
  reContent = regexp.search(content)
  if (reContent and reContent.group() != classEmail) or \
      profanityfilter.is_profane(content) or regexp.search(subject) or \
      profanityfilter.is_profane(subject):
    print(" Email or phone or last name or profanity in subject or content" +
          "send to: " + CLASS_DATA.classes[classIndex].teacher_email)
    ...
    filteredSubject = re.sub(PHONE_EMAIL_NAME_REGEX%lastName,
                             r'*CONTENT_PROBLEM*: \1', censoredSubject,
                             flags=re.IGNORECASE)
    filteredContent = re.sub(PHONE_EMAIL_NAME_REGEX%lastName,
                             r'*CONTENT_PROBLEM*: \1', censoredContent,
                             flags=re.IGNORECASE)
    filteredSubject = "CONTENT_PROBLEM by {0} : {1}".format(
      email.utils.parseaddr(emailFrom)[1], filteredSubject)
    return SendData(CLASS_DATA.classes[classIndex].teacher_email,
                    filteredSubject, filteredContent)
  else:
